...
def save_tree_files(tree_nodes, output_dir, resources, template_name="base.html"):
    ...
    # Obtenemos JS y CSS crudos (incluyendo tags <script>...)
...
def build_imsmanifest(course_title, tree_nodes, output_dir):
    # Registrar namespaces para evitar prefijos 'ns0:' que rompen SCORM
    ...
    # Manifiesto SIN schemaLocation externo para evitar errores de validación en Cloud
    ...
    def recursive_item_builder(parent_xml, nodes):
        for node in nodes:
            ...
            # Item del menú
            item = ET.SubElement(parent_xml, "item", identifier=item_id, identifierref=res_id)
            ET.SubElement(item, "title").text = node['title']
            
            # Recurso físico (scormtype es vital)
...
def build_scorm_package(tree_data, output_zip_path, course_title="Curso SCORM", assets_paths=None):
    ...
    try:
        ...
        print(f"✅ Paquete SCORM generado correctamente: {output_zip_path}")
